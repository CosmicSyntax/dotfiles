...
Scope {
	...
	function updateNightShift() {
		getNightShiftProc.exec(["sh", "-c", "sunsetr status 2>/dev/null | grep -q 'Active' && echo on || echo off"]);
	}
	...
	function toggleNightShift() {
		...
		if (targetState) {
			runSystemCommand("sunsetr restart --background");
		} else {
			runSystemCommand("sunsetr stop");
	...
	function updateBluetooth() {
		getBtProc.exec(["sh", "-c", "bluetoothctl show 2>/dev/null | grep 'Powered:' | awk '{print $2}'"]);
	}

	function toggleBluetooth() {
        let targetState = !root.bluetoothEnabled;
        root.bluetoothEnabled = targetState;
        runSystemCommand(targetState ? "bluetoothctl power on" : "bluetoothctl power off");
    }
	...
	NotificationServer {
		...
		onNotification: notif => {
			...
			// Fallback to appName if desktopEntry is empty
			let dEntry = notif.desktopEntry ? notif.desktopEntry.toString() : "";
			let aName = notif.appName ? notif.appName.toString() : "";
			let resolvedApp = dEntry !== "" ? dEntry : aName;
	...
	function triggerAction(nId, appName) {
		...
		let cleanName = (appName && appName !== "") ? appName.replace(".desktop", "").toLowerCase() : "";
		appLaunchProc.exec([
			"sh", "-c",
			`WINDOW_ID=\$(niri msg --json windows | jq -r --arg app "${cleanName}" '[.[] | select(\$app != "" and ((.app_id // "" | ascii_downcase | contains(\$app)) or (.title // "" | ascii_downcase | contains(\$app)))) | .id] | first'); ` +
			`if [ -n "\$WINDOW_ID" ] && [ "\$WINDOW_ID" != "null" ]; then niri msg action focus-window --id "\$WINDOW_ID"; fi`
		]);
	}
